...
MYSQL *mariadb_dsn_connect(MYSQL *mysql, const char *host, const char *user, const char *passwd, const char *db,
						   unsigned int port, const char *unix_socket, unsigned long clientflag);
void mariadb_dsn_close(MYSQL *mysql);
int mariadb_db_set_connection(MYSQL *mysql, enum enum_server_command command, const char *arg, size_t length,
							  my_bool skipp_check, void *opt_arg);
...
typedef enum { MARIADB_STATE_INIT, MARIADB_STATE_DOWN, MARIADB_STATE_CONNECTED, MARIADB_STATE_ERROR } mariadb_state_t;
...
#define mariadb_handle_exec_base(handle, sql, err)                                                                     \
	mariadb_handle_exec_base_detailed(__FILE__, (char *)__SWITCH_FUNC__, __LINE__, handle, sql, err)
...
static int db_is_up(mariadb_handle_t *handle)
{
	...
	if (handle) {
		...
		if (max_tries < 1) max_tries = DEFAULT_MARIADB_RETRIES;
	}
...
top:
	...
	if (mysql_ping(&handle->con) != 0) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						  "mysql_ping returned bad connection [Error: %s]; reconnecting...\n",
						  mysql_error(&handle->con));
		handle->state = MARIADB_STATE_ERROR;
		if (mariadb_reconnect(&handle->con) != 0) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
							  "mariadb_reconnect returned bad connection -- reconnection failed! [Error: %s]\n",
							  mysql_error(&handle->con));
			goto error;
		}
...
error:
	...
	if (switch_event_create(&event, SWITCH_EVENT_TRAP) == SWITCH_STATUS_SUCCESS) {
		switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Failure-Message",
								"The sql server is not responding for DSN %s [%s][%d]", switch_str_nil(handle->dsn),
								switch_str_nil(err_str), code);
		...
		if (recon == SWITCH_STATUS_SUCCESS) {
			switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Additional-Info",
	...
	if (handle->stored_results) {
		if ((status = mysql_next_result(&handle->con))) {
			if (status > 0) {
				err_str = mariadb_handle_get_error(handle);
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
								  "An error occurred trying to get next for query (%s): %s\n", handle->sql, err_str);
				switch_safe_free(err_str);
	...
	if (res->result) {
		*result_out = res;
		...
		if (mysql_field_count(&handle->con) != 0) {
			...
			err_str = mariadb_handle_get_error(handle);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
							  "An error occurred trying to use result for query (%s): %s\n", handle->sql, err_str);
			switch_safe_free(err_str);
		}
...
switch_status_t mariadb_finish_results_real(const char *file, const char *func, int line, mariadb_handle_t *handle)
{
	...
	do {
		...
		if (res && res->err && !switch_stristr("already exists", res->err) &&
			!switch_stristr("duplicate key name", res->err)) {
			switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, NULL, SWITCH_LOG_ERROR,
...
switch_status_t mariadb_handle_new(switch_cache_db_database_interface_options_t database_interface_options,
								   switch_database_interface_handle_t **dih)
{
	mariadb_handle_t *new_handle = NULL;

	if (!(*dih = malloc(sizeof(**dih)))) { goto err; }

	if (!(new_handle = malloc(sizeof(*new_handle)))) { goto err; }

	memset(new_handle, 0, sizeof(*new_handle));

	if (!(new_handle->dsn = strdup(database_interface_options.connection_string))) { goto err; }
	...
	if (mariadb_handle_connect(new_handle) != SWITCH_STATUS_SUCCESS) {
		if (mariadb_handle_destroy(dih) != SWITCH_STATUS_SUCCESS) { goto err; }
	}
...
switch_status_t mariadb_handle_destroy(switch_database_interface_handle_t **dih)
{
	...
	if (!dih) { return SWITCH_STATUS_FALSE; }
...
switch_status_t mariadb_flush(mariadb_handle_t *handle)
{
	...
	if (handle->stored_results) {
		if (mysql_next_result(&handle->con) != 0) { goto done; }
	}
	...
	while ((tmp = mysql_store_result(&handle->con)) != NULL) {
		...
		if (mysql_next_result(&handle->con) != 0) break;
	}

	if (x) { switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG10, "Flushing %d results\n", x); }
												  ...
												  mariadb_handle_t *handle, const char *sql, char **err)
{
	...
	if (mariadb_send_query(handle, sql) != SWITCH_STATUS_SUCCESS) {
		...
		if (mariadb_finish_results(handle) != SWITCH_STATUS_SUCCESS) { db_is_up(handle); }
		goto error;
	}
...
switch_status_t mariadb_handle_exec_detailed(const char *file, const char *func, int line, mariadb_handle_t *handle,
											 const char *sql, char **err)
{
	if (mariadb_handle_exec_base_detailed(file, func, line, handle, sql, err) == SWITCH_STATUS_FALSE) { goto error; }
...
switch_status_t database_handle_exec_string(switch_database_interface_handle_t *dih, const char *sql, char *resbuf,
											size_t len, char **err)
{
	mariadb_handle_t *handle;
	...
	if (mariadb_handle_exec_base(handle, sql, err) == SWITCH_STATUS_FALSE) { goto error; }

	if (mariadb_next_result(handle, &result) == SWITCH_STATUS_FALSE) { goto error; }
	...
	if (handle->affected_rows <= 0) { goto done; }
	...
	if (mariadb_finish_results(handle) != SWITCH_STATUS_SUCCESS) { sstatus = SWITCH_STATUS_FALSE; }
...
switch_status_t mariadb_handle_callback_exec_detailed(const char *file, const char *func, int line,
													  switch_database_interface_handle_t *dih, const char *sql,
		...
		for (row = 0; row < result->rows; ++row) {
			...
			for (col = 0; col < result->cols; ++col) {
				...
				if (field) {
					...
					switch_log_printf(SWITCH_CHANNEL_ID_LOG, file, func, line, NULL, SWITCH_LOG_ERROR,
									  "ERR: Column number %d out of range\n", col);
				}
			}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_mariadb_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	database_interface = (switch_database_interface_t *)switch_loadable_module_create_interface(
		*module_interface, SWITCH_DATABASE_INTERFACE);
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_mariadb_shutdown) { return SWITCH_STATUS_UNLOAD; }
